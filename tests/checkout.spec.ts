...
test.describe('tests of buying products from cart, and finalize checkout', () => {
  ...
  test('Buy products - happy path', async ({
    page,
    checkoutStepOnePage,
    checkoutStepTwoPage,
    ...
    await checkoutStepOnePage.enterCustomerData(
      'firstname',
      'lastname',
      '12-345',
    );
  ...
  test('Buy products - unhappy path invalid zip code', async ({
    page,
    checkoutStepOnePage,
  }) => {
    await checkoutStepOnePage.enterCustomerData(
    ...
    expect
      .soft(page.url())
      .toBe('https://www.saucedemo.com/checkout-step-one.html');
    await expect.soft(checkoutStepOnePage.checkoutTitle).toBeVisible();
    await expect.soft(checkoutStepOnePage.error).toBeVisible();
  });

  test('Buy products - unhappy path - empty customer information fields', async ({
    page,
    checkoutStepOnePage,
  }) => {
    await checkoutStepOnePage.goToNextStepOfCheckout();
  ...
  test('Buy products - back to change client information', async ({
    page,
    checkoutStepOnePage,
    checkoutStepTwoPage,
  }) => {
  ...
  test('Buy products - back to cart', async ({
    cartPage,
    page,
    checkoutStepOnePage,
  }) => {
