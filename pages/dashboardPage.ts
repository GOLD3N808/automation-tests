...
this.sortingDropdownList = page.locator('[data-test="product-sort-container"]')
...
async sortByAtoZ(){
    ...
    let texts = await Promise.all(nameElements.map(async element => await element.textContent()))
    let isSorted = texts.every((text, i, array) => i === 0 || array[i - 1].localeCompare(text) <= 0)
    return isSorted 
}
...
async sortByPriceAsc(){
    ...
    let prices = await Promise.all(priceElements.map(async element => {
    ...
    let isSorted = prices.every((price, i, array) => i === 0 || array[i - 1] <= price)
    return isSorted
}
